...
No data repair or on-load conversion happens here. The file view retains the
supplied body states. Unknown source frame/COM
...
def load_views(
    ...
    if source_format == "t800_isaac_v1":
        ...
        if len(joint_names) != 25 or set(source_joints) != set(joint_names):
            raise ValueError("Source joint names must match the selected T800 joints")
        if model.nbody != 31 or set(bodies[1:]) != set(SOURCE_BODY_NAMES):
            raise ValueError("Model must contain the 30 named T800 bodies plus world")
...
def validate_motion(
    ...
    fk = tracking_fk(model, view.joint_names, fk_input)
