...
from engineai_rl_unilab.tasks.t800.motion_adapter import STATE_FIELDS
...
def test_source_is_retained_and_reference_matches_training(names):
    digest = hashlib.sha256(SOURCE.read_bytes()).digest()
    model, views = load_views(SOURCE, MODEL, names, "t800_isaac_v1")
    training = T800MotionLoader(str(SOURCE), model_file=str(MODEL), joint_names=names)
    for field in STATE_FIELDS:
        np.testing.assert_array_equal(
            getattr(views["reference"].data, field), getattr(training, field)
        )
    ...
    assert validate_motion(views["reference"], model).status == "PASS"
